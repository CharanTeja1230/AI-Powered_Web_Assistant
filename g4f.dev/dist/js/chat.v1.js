...
window.addEventListener("hashchange", async (event) => {
    ...
    if (locationHash == "login") {
        window.location.href='/login.html?redirect='+encodeURIComponent(location.href.split('#')[0])+'&conversation='+encodeURIComponent(window.conversation_id);
        return;
    }
...
function load_provider_login_urls(providersListContainer, providers = []) {
    for (const provider of providers) {
        ...
        const apiKeyLink = ["Pollinations", "HuggingFace", "Airforce"].includes(provider.name)
            ? `<a href="/login.html?provider=${login_provider}&redirect=${encodeURIComponent(window.location.href.split("#")[0])}" title="${framework.translate("Login to")} ${framework.escape(label)}">${framework.translate('Login')}</a>`
...
async function load_version() {
    window.title = 'LUMO Chat';
    document.title = window.title;
    const ver = document.getElementById("version_text");
    if (ver) {
        ver.innerHTML = "LUMO";
    }
}
...
function cloudSyncLoginRedirect(provider = null) {
    ...
    window.location.href = `/login.html?redirect=${returnUrl}${conversation}${providerParam}`;
}
